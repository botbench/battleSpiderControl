...
declare namespace BLECentralPlugin {
  ...
  interface PeripheralCharacteristic {
    ...
    interface PeripheralData {
        ...
        id: string;
        rssi: number
        advertising: ArrayBuffer|any; // THIS IS DIFFERENT BETWEEN ANDROID AND IOS, PARSE DIFFERENTLY
    }
    ...
    export interface BLECentralPluginStatic {
        ...
        autoConnect(device_id:string, connectCallback: (data: PeripheralDataExtended) => any, disconnectCallback: () => any): void;
        ...
        //request a change the MTU size(ble packet size) 
        //android only
        requestMtu(device_id: string, mtu: number, success: () => any, failure: () => any): void;

        //refresh device characteristic chache 
        //android only
        refreshDeviceCache(device_id: string, timeout_in_ms: number, success: () => any, failure: () => any): void;
